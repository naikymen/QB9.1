...
query = session.query(Ptms.columns.id, Ptms.columns.description, Ptms.columns.type).\
    filter(text("type='cross-link'")).\
    order_by(Ptms.columns.id)


print("{: <40} {: <40} {: <40} {: <40}".format(*['Clean', 'Raw', 'Id', 'Type\n']))
